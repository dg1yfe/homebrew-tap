...
macOS only, deliberately: the release tarballs for Linux are statically linked
and run anywhere, so a formula would add a step without adding reach. (`brew
style` also rejects a per-platform `url` inside `on_linux`, which is what a
multi-platform binary formula would need.)
